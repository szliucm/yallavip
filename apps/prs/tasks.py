...
my_access_token = "EAAcGAyHVbOEBACfnryb4knGZCXesSZBUG6GRy5zaQZBW9elZB7pg8jghr6DZAxeh49XVcYQGdm27qyuNpRPcLgH7yIh7k6BrUfLCCdBR2eUZA0L0Mv9MH8yqgPrVZAbk7FsT9ceF3O2pM066jHr3RS0TqC5EH8mOq3vocAxOpCYsdHWAKIzmIJS7dToiUUcvOoZD"
...
def sync_yallavip_album(album_name=None):
    ...
    if album_name:
        ...
        lightinalbums_all = LightinAlbum.objects.filter(
            Q(lightin_spu__sellable__gt=0) | Q(lightin_sku__o_sellable__gt=0),
            published=False, publish_error="无", material=True, yallavip_album__isnull=False)

    albums = lightinalbums_all.values_list('yallavip_album').distinct()
    print("有%s个相册待更新" % (albums.count()))
    for album in albums:
        lightinalbums = lightinalbums_all.filter(yallavip_album=album).order_by('?')[:9]
        #print("相册%s 批次 %s 有%s 个图片待发" % (batch_no[0], batch_no[1], lightinalbums.count()))
