...
def auto_smscode():
    ...
    for row in orders_tocheck:
        ...
        if row.customer:
            ...
            conversation = row.customer.customer_conversation.values_list("conversation",flat=True).distinct()
            if len(conversation) > 0 and conversation[0]:
...
def sync_yallavip_album(page_no=None):
    ...
    print("有%s个相册图片待更新" % (lightinalbums_all.count()))
