...
def sync_Shipped_order_shopify():
    ...
    orders = Order.objects.filter(status = "open", wms_status__in= ["W"],logistic_no__isnull=False, order_id__isnull=False)
...
def update_shopify_variant():
    ...
    variants = ShopifyVariant.objects.filter(inventory_policy="continue").distinct().orderby("variant_no").values_list("variant_no",flat=True)
