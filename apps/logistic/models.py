...
class Package(models.Model):
    ...
    logistic_track_code = models.CharField(choices=TRAIL_STATUS,verbose_name='物流轨迹代码', max_length=100, null=True,
                                              blank=True)
...
class LogisticResendTrail(Package):
    ...
    def cal_resend_date(self):
        ...
        if self.file_status =="OPEN":
            now = datetime.now().replace(tzinfo=cst_tz)
            if self.resend_start_time is not None:
...
class LogisticTrail(models.Model):
    package = models.ForeignKey(Package, related_name='package_trail', null=True, on_delete=models.CASCADE,
                                verbose_name="包裹")
    waybillnumber = models.CharField(u'物流追踪号', default='', max_length=100, blank=True, null=True)
    trail_status  = models.CharField(u'发生状态', default='', max_length=500, blank=True, null=True)
    trail_statuscnname = models.CharField(u'发生状态_zh', default='', max_length=500, blank=True, null=True)
    trail_time = models.DateTimeField(u'发生时间', auto_now=False, blank=True, null=True)
    trail_locaiton= models.CharField(u'发生地点', default='', max_length=100, blank=True, null=True)
